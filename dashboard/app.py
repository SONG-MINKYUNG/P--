...
for var in continuous_vars:
    ...
    # 초기값은 원래 데이터프레임의 값의 중간값으로 설정
    initial_value = int((original_min + original_max) / 2)
    ...
    selected_vars[var] = st.sidebar.slider(
        ...
        value=initial_value,  # 초기값은 원래 데이터프레임 기반
        key=f"slider_{var}"
    )
...
# 예상 공유차량 이용건수 초기값 저장 및 업데이트 (단지 변경 시 초기화)
if "current_complex" not in st.session_state:
    st.session_state["current_complex"] = selected_complex  # 현재 단지명 저장
    st.session_state["initial_predicted_value"] = model.predict(filtered_data[feature_columns])[0]
elif st.session_state["current_complex"] != selected_complex:
    # 단지가 변경된 경우 초기값 및 상태 초기화
    st.session_state["current_complex"] = selected_complex
    st.session_state["initial_predicted_value"] = model.predict(filtered_data[feature_columns])[0]

# 변수 초기값 설정 (단지의 실제 값 기반)
